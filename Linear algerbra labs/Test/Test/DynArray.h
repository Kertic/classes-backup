...
public:
	...
	DynArray<Type>& operator=(const DynArray<Type>& that) {
		if (this != &that)
		{
			Type * tempArray = new Type[that.size()];
	...
	// Notes : 	This will add items to a new array UNLESS the item is in the index we've specified to remove. The size goes down by one, and the capacity stays the same
	/////////////////////////////////////////////////////////////////////////////
	...
	void remove(int index) {
		...
		for ( ; i < _size; i++)
		{
			...
			{
				i++;
				break;//We break here because we've found the index to skip
			}
		}

		for ( ; i <_size; i++)
		{
			temp[i - 1] = _array[i];//This will set 
		}
		_size--;
