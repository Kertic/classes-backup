...
//_BaWPixel needs to be a pixel with identical RGB values, and it will assume the lower the value, the more color you want.
unsigned int ModulateColor(unsigned int _BaWPixel, unsigned int _color) {

	float ratio = (float)(1.0f- ((_BaWPixel << 8) >> 24) / 255.0f);

	float red = (float)((_color << 8) >> 24);
	float green = (float)((_color << 16) >> 24);
	float blue = (float)((_color << 24) >> 24);
	float alpha = (float)(_color >> 24);

	float newRed = red * ratio;
	float newGreen = green * ratio;
	float newBlue = blue * ratio;

	return ConvertColorRGBA(newRed, newGreen, newBlue, alpha);
...
#pragma endregion
	...
	if (startPrimary < endPrimary) {
		for (int i = startPrimary; i < endPrimary; i++)
		{
			...
			float currZ = 1.0f;//Lerp(augmentedPoint2.z, augmentedPoint1.z, ratio);
	...
	else
	{
		for (int i = startPrimary; i > endPrimary; --i)
		{
			...
			float currZ = 1.0f; //Lerp(augmentedPoint1.z, augmentedPoint2.z, ratio);
			PaintPixel(pixelArray, From2Dto1D((int)currX, (int)currY, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT), color, currZ,  zbuffer, zbuffersize);
		}
	}
		...
		for (int y = minY; y < maxY; y++)
		{
			...
			else
			{
				...
				if (!(x < 0 || x > SCREEN_WIDTH || y < 0 || y > SCREEN_HEIGHT)) {
					//CHANGE ME TO INCLUDE DEPTH
					PaintPixel(pixelArray, From2Dto1D(x, y, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT), ConvertColorRGBA((unsigned int)redCh, (unsigned int)greenCh, (unsigned int)blueCh, 255), currentZ, zbuffer, zbuffersize);
				}
